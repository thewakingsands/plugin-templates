...
function swapLang(val) {
    ...
    for (var i in lang) {
        $('#' + i).find('img').attr('src', 'config/haeruskin/images/ui/' + i + tmp + '.svg');
...
function viewTab(val, flag) {
    if (val == 'edit') {
        ...
        Button('menu', 0);
        $('#menu img').attr('src', 'config/haeruskin/images/ui/close.svg');
        $('#wrap').css('height', 'calc(100% - ' + ($('.navbar-fixed').height() / 10 - 0.1) + 'rem)')
    } else {
        $('#menu').find('input').prop('checked', true)
        Button('menu', 0);
...
function createEditTable() {
    ...
    for (var i in list) {
        ...
        var div = '<div id="' + i + '"><table class="tablebody edit"><tr>' +
            ...
            '<td class="cell_2" name="DPS"><img src="config/haeruskin/images/ui/check_' + a.DPS + '.svg"></td>' +
            '<td class="cell_2" name="HPS"><img src="config/haeruskin/images/ui/check_' + a.HPS + '.svg"></td>' +
            '<td class="cell_2" name="BM"><img src="config/haeruskin/images/ui/check_' + a.BM + '.svg"></td></tr></table><div class="editbg"></div><div class="divider"></div></div>'
        html += div;
    }
    $('#editBody').html(html)
    $('.tablebody.edit td.cell_2').on('click', function () {
        ...
        if ($(this).find('img').attr('src').split('_')[1] == 'true.svg') {
            ...
            $(this).find('img').attr('src', 'config/haeruskin/images/ui/check_false.svg');
        } else if ($(this).find('img').attr('src').split('_')[1] == 'false.svg') {
            list[key][$(this).attr('name')] = true
            $(this).find('img').attr('src', 'config/haeruskin/images/ui/check_true.svg');
        } else { }
    ...
    switch (id) {
        case 'menu':
            if ($('#' + id).find('input').prop('checked') == false) {
                ...
                $('#menu img').attr('src', 'config/haeruskin/images/ui/menu.svg');
                ...
                if (localStorage.getItem("thema") == 5)
                    ...
                    $('#zone').css({
                        'background-image': 'url(./config/haeruskin/images/handle.svg)',
        ...
        case 'history':
            if ($('#' + id).find('input').prop('checked') == false) {
                $('#history img').attr('src', 'config/haeruskin/images/ui/close.svg');
                ...
                $('#menuTable tr:first div').text('');
                $('#' + localStorage.getItem('tab')).find('div').text('★');
                $('#history img').attr('src', 'config/haeruskin/images/ui/history.svg');
                $('#' + id).find('input').prop('checked', false);
            }
            break;
        case 'end':
...
function addData(name, td, a) {
    switch (name) {
        case 'Job':
            if (a[name] != undefined) return '<img src="./config/haeruskin/images/glow/' + a[name].toLowerCase() + '.png" class="pngIcon"/>';
            else return ''
        case 'Name':
...
function historyAddRow() {
    ...
    var tr = table.insertRow();
    var td = tr.insertCell();
    td.innerHTML = '<img src="./config/haeruskin/images/ui/eye.svg" style="width:1.5rem"/>';
    td.className = "cell_0";
    td.id = "viewIcon";
    var td = tr.insertCell();
        ...
        click: function () {
            ...
            for (var i in encounterArray) {
                if (listName == encounterArray[i].combatKey) {
                    ...
                    $(this).find('#viewIcon').html('<img src="./config/haeruskin/images/ui/eye.svg" style="width:1.5rem"/>');
