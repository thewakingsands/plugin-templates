...
class DeathReportLive {
  ...
  setDeathReport(report) {
    ...
    for (const event of report.parseReportLines()) {
      this.AppendDetails(detailsDiv, event.timestampStr, event.currentHp?.toString(), event.amountStr, event.amountClass, event.icon, event.text);
    }
  }
  AppendDetails(detailsDiv, timestampStr, currentHp, amount, amountClass, icon, text) {
    ...
    if (amountClass !== undefined) damageElem.classList.add(amountClass);
...
class OopsyLiveList {
  ...
  items = [];
  itemIdxToListener = {};
  constructor(options, scroller, partyTracker) {
    this.options = options;
  ...
  OnMistakeObj(timestamp, m) {
    ...
    if (report) this.deathReport?.queue(report);
...
const addDebugInfo = (collector, numMistakes) => {
  ...
  const types = ['death', 'fail', 'warn', 'pull'];
...
user_config/* default.getUserConfigLocation */.Z.getUserConfigLocation('oopsyraidsy', oopsy_options, () => {
  ...
  if (typeof params.get('debug') === 'string') addDebugInfo(mistakeCollector, 2200);
...
/******/ 	var __webpack_exports__ = __webpack_require__.O(undefined, [727], () => (__webpack_require__(793)))
